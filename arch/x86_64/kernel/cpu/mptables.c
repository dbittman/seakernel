...
static int imps_get_checksum(unsigned start, int length)
{
	unsigned sum = 0;
	while (length-- > 0) 
		sum += *((unsigned char *) (addr_t)(start++));
	return (sum&0xFF);
}
...
static int imps_read_config_table(addr_t start, int count, int do_)
{
	int n=0;
	while (count-- > 0) {
...
static void imps_read_bios(struct imps_fps *fps_ptr)
{
	...
	if (fps_ptr->cth_ptr) {
		...
		cth_start = ((addr_t) local_cth_ptr) + sizeof(struct imps_cth);
...
int imps_scan_mptables(addr_t addr, unsigned len)
{
	addr_t end = addr+len;
	struct imps_fps *fps_ptr = (struct imps_fps *)addr;
	while((addr_t)fps_ptr < end) {
		if(fps_ptr->sig == IMPS_FPS_SIGNATURE)
			kprintf(":: %x %x %x %d %d %d\n", fps_ptr, end, sizeof(struct imps_fps), fps_ptr->length, fps_ptr->spec_rev, imps_get_checksum((addr_t)fps_ptr, 16));
		if (fps_ptr->sig == IMPS_FPS_SIGNATURE
		 && fps_ptr->length == 1
		 && (fps_ptr->spec_rev == 1 || fps_ptr->spec_rev == 4)
		 && !imps_get_checksum((addr_t)fps_ptr, 16)) {
