...
{
	tree->root = insert(tree->root, key, ptr);
}

void *bptree_search(bptree *tree, unsigned key)
{
	record *r = find(tree->root, key, 0);
	if(!r) return 0;
	return r->value;
}
...
{
	tree->root = destroy_tree(tree->root);
}
...
int get_neighbor_index( node * n ) {
	...
	// Error state.
	panic(PANIC_NOSYNC, "Search for nonexistent pointer to node in parent.\n");
	return 0;
}
