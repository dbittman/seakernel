...
void aes_x86_128_encrypt_block(unsigned char *plaintext, unsigned char *ciphertext, unsigned char *round_keys)
{
	...
	/* since x86 32-bit seems to only have 8 SSE registers, we can't load
	 * every round key at once. But we can load the first 6 round keys */
	...
	/* ...and do the first few rounds with them */
	asm("pxor %xmm0, %xmm15; \
	     aesenc %xmm1, %xmm15;\
	     aesenc %xmm2, %xmm15;\
	     aesenc %xmm3, %xmm15;\
