...
static unsigned ioapic_rw(struct ioapic *l, int rw, unsigned char off, unsigned val)
{
...
void calibrate_lapic_timer(unsigned freq)
{
	...
	set_lapic_timer(lapic_timer_start / 2); /* don't know why, but this is needed for the timing to be right */
}
